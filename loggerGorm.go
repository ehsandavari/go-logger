...
func (r *sGormLogger) Info(ctx context.Context, str string, args ...any) {
	...
	r.iLogger.WithEvent("gorm").WithAny("args", args).Debug(ctx.(*contextplus.Context), str)
}
...
func (r *sGormLogger) Warn(ctx context.Context, str string, args ...any) {
	...
	r.iLogger.WithEvent("gorm").WithAny("args", args).Warn(ctx.(*contextplus.Context), str)
}
...
func (r *sGormLogger) Error(ctx context.Context, str string, args ...any) {
	...
	r.iLogger.WithEvent("gorm").WithAny("args", args).Error(ctx.(*contextplus.Context), str)
}
...
func (r *sGormLogger) Trace(ctx context.Context, begin time.Time, fc func() (string, int64), err error) {
	...
	case err != nil && r.LogLevel >= gormLogger.Error && (!r.IgnoreRecordNotFoundError || !errors.Is(err, gorm.ErrRecordNotFound)):
		sql, rows := fc()
		if rows != -1 {
			r.iLogger.WithEvent("gorm").WithError(err).WithDuration("elapsed", elapsed).WithInt64("rows", rows).WithString("sql", sql).Error(ctx.(*contextplus.Context), "trace")
			break
		}
		r.iLogger.WithEvent("gorm").WithError(err).WithDuration("elapsed", elapsed).WithString("sql", sql).Error(ctx.(*contextplus.Context), "trace")
	case r.SlowThreshold != 0 && elapsed > r.SlowThreshold && r.LogLevel >= gormLogger.Warn:
		sql, rows := fc()
		if rows != -1 {
			r.iLogger.WithEvent("gorm").WithDuration("elapsed", elapsed).WithInt64("rows", rows).WithString("sql", sql).Warn(ctx.(*contextplus.Context), "trace")
			break
		}
		r.iLogger.WithEvent("gorm").WithDuration("elapsed", elapsed).WithString("sql", sql).Warn(ctx.(*contextplus.Context), "trace")
	case r.LogLevel == gormLogger.Info:
		sql, rows := fc()
		if rows != -1 {
			r.iLogger.WithEvent("gorm").WithDuration("elapsed", elapsed).WithInt64("rows", rows).WithString("sql", sql).Info(ctx.(*contextplus.Context), "trace")
			break
		}
		r.iLogger.WithEvent("gorm").WithDuration("elapsed", elapsed).WithString("sql", sql).Info(ctx.(*contextplus.Context), "trace")
	}
}
